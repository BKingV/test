...
def extract_themes_and_questions(doc):
    ...
    themes = {}
    st.write("📌 Количество таблиц в документе:", len(doc.tables))  # Проверяем, сколько таблиц есть
    st.write("📌 Содержимое первой таблицы:", [[cell.text for cell in row.cells] for row in doc.tables[0].rows])
